...
const createHttpException = (status: StatusCodes, message: string, state: any) => ({
  status,
  message,
  state
});
...
const configure = (connector: OAuthConnector) => {
  const settingsManagers = async (ctx: IntegrationContext, state: FunctionState, data: FunctionData) => {
    ...
    if (!data.fusebit_skip_settings_managers) {
      ...
      if (settingsManagers.length > stage) {
        ...
        return redirect(ctx, state, data, settingsManagers[stage], 'settingsManagers');
      }
    }
    ...
    try {
      ...
      throw createHttpException(StatusCodes.INTERNAL_SERVER_ERROR, `Error initializing new user: ${e.message}`, state);
    }
  ...
  const authInit = async (ctx: IntegrationContext, state: FunctionState, data: FunctionData) => {
    ...
    const rawVariant = data?.query?.variant;
    const variant = typeof rawVariant === 'string' ? rawVariant : undefined;
    const variantCtx = connector.withVariantConfig(ctx, variant);
  ...
  const authCallback = async (ctx: IntegrationContext, state: FunctionState) => {
    ...
    if (cookieHeader) {
      cookieHeader.split(';').forEach(cookie => {
        ...
        if (index > 0) {
          if (cookie.substring(0, index).trim().toLowerCase() === SAVED_STATE_COOKIE) {
    ...
    if (serializeState(state) !== savedState) {
      ...
      throw createHttpException(StatusCodes.UNAUTHORIZED, 'Error processing authorization code response.', state);
    }
    ...
    if (ctx.query?.code) {
      try {
        const rawVariant = state.data?.query?.variant;
        const variant = typeof rawVariant === 'string' ? rawVariant : undefined;
        const variantCtx = connector.withVariantConfig(ctx, variant);
